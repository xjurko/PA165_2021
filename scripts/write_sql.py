...
def writeUserRatings(ratings: List[Rating], append=False):
  ...
  with open("./imdb/out/user_ratings.sql", writeMode(append)) as file:
    for c in chunks(ratings):
      file.write(f"INSERT INTO MovieRating (rating, movie_id, user_id) VALUES ")
      values = [f"('{r.rating}', {r.movieId}, {r.userId})" for r in c]
      values = ",\n".join(values)
      file.write(f"{values};\n")


def writeUsers(users: List[User], append=False):
  """create table Users (
       id bigint generated by default as identity,
  ...
  with open("./imdb/out/users.sql", writeMode(append)) as file:
    for c in chunks(users):
      file.write(f"INSERT INTO Users (id, email, isAdmin, name, passwordHash) VALUES ")
      values = [f"({u.id}, '{u.email}', {str(u.isAdmin).capitalize()}, '{u.name}', '{u.passHash}')" for u in c]
      values = ",\n".join(values)
      file.write(f"{values};\n")
